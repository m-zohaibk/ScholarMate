...
export default function TeacherQuizGenerator() {
  ...
  const handleSyllabusFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast({ title: 'File too large', description: 'Use a syllabus file smaller than 2MB.', variant: 'destructive' });
      return;
    }
  ...
  return <div className="max-w-5xl mx-auto space-y-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6"><Card id="syllabus" className="border-none shadow-sm"><CardHeader><CardTitle className="text-lg">Syllabus / Topics</CardTitle><CardDescription>Paste your syllabus or import a plain-text syllabus file.</CardDescription></CardHeader><CardContent className="space-y-4"><input ref={syllabusFileRef} type="file" accept=".txt,.md,text/plain" className="hidden" onChange={handleSyllabusFile} /><Button type="button" variant="outline" className="w-full border-dashed" onClick={() => syllabusFileRef.current?.click()}><FileUp className="w-4 h-4 mr-2" />Import syllabus text</Button><Textarea placeholder="Enter syllabus details here..." className="min-h-[200px] bg-background/50 border-dashed border-2" value={syllabus} onChange={(event) => setSyllabus(event.target.value)} /><Button className="w-full h-12 text-lg font-headline" disabled={loading || !syllabus.trim()} onClick={handleGenerate}>{loading ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" />Generating...</> : <><Sparkles className="w-5 h-5 mr-2" />Generate Quiz</>}</Button></CardContent></Card>
