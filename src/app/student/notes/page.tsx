...
export default function StudentNotesGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/student/notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studyMaterialDataUri: fileData, detailLevel }) });
      if (!response.ok) throw new Error('notes-generation-failed');
      const result = await response.json() as StudentStructuredNotesOutput;
      ...
      toast({ title: 'Generation failed', description: 'Try a clearer image or a smaller PDF.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
