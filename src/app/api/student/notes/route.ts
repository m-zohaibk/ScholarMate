...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isGeminiPdfUri(input.studyMaterialDataUri)) {
      console.log('[API] Student notes Gemini file generation started:', { fileName: input.fileName, hasFileUri: true, mediaContentType: 'application/pdf' });
      ...
      console.log('[API] Student notes Gemini file generation completed:', { fileName: input.fileName });
    ...
    console.error('[API] Student notes generation failed:', { name: error instanceof Error ? error.name : 'UnknownError', message: error instanceof Error ? error.message : String(error) });
    ...
    return NextResponse.json({ error: 'The document could not be analyzed. Check the document format and try again.' }, { status: 500 });
  }
}
